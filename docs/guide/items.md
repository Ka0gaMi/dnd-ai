...
`inventory {op: add, campaign_id, name}`. Use the SRD spelling where one exists and the item arrives
with its category, damage or armour value, properties and weight already written in.
...
**An item you invented must name its rarity.** `inventory {op: add, name: "Whispering Blade", magic:
{rarity: "rare", attunement: "by a Rogue", bonus: 1, base: "Longsword"}}`. A `bonus` also needs
`base`, the SRD weapon, armour or shield the item is a version of; one already named for SRD
equipment ("Shield", "Chain Mail") needs none. Without `rarity` the call is refused and the error
...
`inventory {op: list}` still show everything, with `true_name` beside the masked one. Two things
reveal it: a short rest spent studying it
(`rest{kind: "short", identify: ["<item>"]}`) or the Identify spell
(`spells {op: spend_slot, level: 1, spell: "Identify", target_item: "<item>"}`).
...
Until an item that asks for attunement is attuned, **its bonus does nothing**. `inventory`'s `add` and
`equip` ops say so in `attunement_note` when they hand over such an item.
...
An item may also carry a `mechanics` block -
`inventory {op: add, magic: {mechanics: {resistances: ["fire"]}}}`
...
`inventory {op: use, campaign_id, item, charges}` spends charges - `charges` defaults to 1. The reply
says what is left, when they come back and repeats the item's own rules text.

**`inventory {op: use}` does not cast anything.** A wand's spell is yours to narrate, and if the
player is the one casting it goes through `use_action` in a fight or `spells {op: spend_slot}` outside
one. `inventory {op: use}` only moves the charges.

Recharging happens by itself:

- `dawn` / `dusk` items roll their recharge dice the moment the campaign clock passes 06:00 or 18:00
  - through `time {op: advance}`, or through the hours a rest costs. The roll is logged like any other.
...
- `unknown` means the bundled SRD text carries the charges but not the recharge rule, as it does for
  ...
  with `inventory {op: use, item, restore: 3}`, which hands the charges back and logs it as your
  ruling, or settle it for good with
  `inventory {op: add, name, magic: {charges: {current, max, recharge: "dawn"}}}`.

At zero charges `inventory {op: use}` refuses and names the recharge. Read that out: the item cannot
be used again until then.

## Containers

`inventory {op: add, name: "Rope, Hempen", into: "Backpack"}` puts the item inside a container the
character already carries. `inventory`'s `remove`, `equip`, `sell` and `use` ops all find items
inside containers, by name or by the item's `id`. `inventory {op: list}` shows contents nested under
their container.
...
the top level; put it back with `inventory {op: remove}` and `inventory {op: add, into: ...}` if the
story needs it stowed.
...
`inventory {op: add, name: "Saddlebag", container: {capacity_lb: 30}}`.
...
`inventory {op: gold, delta: -15, reason: "..."}` still works and still means gold pieces.
`inventory {op: gold, coins: {sp: -5}, reason: "..."}` moves exact denominations. Pass one or the
other; a call with both is refused.
...
`inventory {op: sell, campaign_id, item, qty}`. The default price is **half the SRD list price**, the
2024 rule of thumb, and the reply names the rule it used so you can say it out loud. **Every magic
item needs a `price_gp` from you** - the SRD prices the Chain Mail, never the +2 Chain Mail - and so
does anything you want to haggle to a different number.

It refuses an item still equipped or still attuned - take it off with `inventory {op: equip}`, end
the attunement on a short rest, or pass `force: true` when the story means them to hand it over
anyway. A gift, a theft or a broken item is `inventory {op: remove}`, not this.

## Rules the engine does not own

These are yours. The engine will not do them and will not stop you.

- **Casting from a wand or staff.** `inventory {op: use}` spends the charge; the spell, its save DC,
  its targets and its effect are narrated by you and run through `use_action` or
  `spells {op: spend_slot}`.
...
- **Item interactions.** A Bag of Holding inside a Portable Hole, a potion mixed with another, an
  item destroyed by fire: narrate it and use `inventory {op: remove}`.
