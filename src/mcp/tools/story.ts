...
export function registerStoryTools(server: McpServer, db: Db): void {
  registerOpTool(server, 'story', {
    ...
    description:
      "Outline writes the story's shape: the premise in one paragraph, the ending you steer towards, and DM-only secret notes the player must not see. Call it at the start, after create_campaign and the premise, and when the story changes direction; secret_notes never reaches the player unless their spoiler toggle is on. Follow it with act and open_chapter. Act adds the next large movement a run of chapters belongs to, with its goal. Call it when you write the outline (two to four acts is plenty) and when a sandbox grows a movement; acts are numbered as you add them, and an act becomes active when a chapter opens inside it. open_chapter opens a chapter, the unit of play between recaps. Call it at the start and whenever no chapter is open; everything recorded afterwards is stamped with it. It refuses while one is open: closing it needs a summary, so use advance_chapter. Pass act_id to place it, or leave it out to continue the current act. advance_chapter closes the open chapter with a recap and opens the next in one call. Call it when a chapter is over, after the scene's checkpoint {op: save}, not instead of it. The summary is 3-6 sentences and becomes the chapter recap every later briefing carries; write what changed and what hangs. Pass act_id when the new chapter starts a new act. It needs a chapter to close; with none open it refuses and saves nothing, so call open_chapter first.",
    fields: {
