...
export const GUIDE_SECTIONS = ['story', 'rolls', 'combat', 'codex', 'progression', 'homebrew', 'items'] as const;
...
export function registerGuideTools(server: McpServer, db: Db): void {
  server.registerTool(
    'read_guide',
    {
      ...
      description:
        'Returns the detailed procedure for one area of play: story (acts, chapters, threads, clues, rumours, time, secrets, the journal), rolls (who rolls what and how the player rolls their own dice), combat (the whole fight loop), codex (people, places and NPC voices), progression (levelling, homebrew and house rules), homebrew (how to write a mechanic the engine can run) or items (magic items, attunement, charges, containers, coins and selling). Call it before you run an area for the first time in a chat, and whenever you are unsure which tool comes next - it is longer and more exact than the tool descriptions. It reads a bundled page and changes nothing.',
