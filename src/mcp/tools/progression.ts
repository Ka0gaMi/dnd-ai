...
export function registerProgressionTools(server: McpServer, db: Db): void {
  server.registerTool(
    'note_play',
    {
      ...
      description:
        'Records one line about how the player solved something, with tags from a fixed list. Call it when they do something characteristic - rigging a trap, talking their way past a guard, setting the room on fire - one or two lines per session, in their own words where you can. Each note is logged as a play_note event. The notes build the play profile that get_play_profile returns and that your level-up suggestions should be based on, so this is how the game learns to offer them things they will actually enjoy.',
      inputSchema: {
  ...
  server.registerTool(
    'get_play_profile',
    {
      title: 'Get the play profile',
      description:
        "How this character actually plays: the tags you have noted with their counts, the best examples in your own words, and what the engine has seen - the skills rolled, the attacks and actions used in fights, and the effects left on the battlefield. Read it before you suggest a level-up option or invent a feature for them, so what you offer follows how they play instead of what their class says. It changes nothing.",
      inputSchema: { campaign_id: z.number().int(), character_id: CHARACTER_ID },
      annotations: { ...READS },
    },
    (input) => reply(db, input.campaign_id, playProfile(db, input.campaign_id, input.character_id)),
  );
  ...
  server.registerTool(
    'propose_feature',
    {
      ...
      inputSchema: {
        ...
        character_id: CHARACTER_ID,
        name: z.string(),
        text: z.string().describe('How the feature reads on the sheet.'),
        mechanics: MECHANICS,
        clauses: CLAUSES.optional(),
        justification: z.string().describe('Why they have earned it; the player sees this in the dialog.'),
        allow_over_budget: z
          .boolean()
          .optional()
          .describe('The player asked for it knowing it is too strong. Never set this on your own.'),
      },
      annotations: { ...WRITES },
    },
    async (input) => {
  ...
  server.registerTool(
    ...
    (input) => {
      ...
      if (entry.kind !== 'feature' && entry.kind !== 'feat') {
        return reply(db, input.campaign_id, {
          status: 'refused',
          message: `revise_mechanics restates a feature or a feat; "${entry.name}" is a ${entry.kind}. A spell is re-proposed with propose_spell, a subclass with propose_subclass.`,
        });
      }
      ...
      if (report.verdict === 'over_budget' && rulesMode !== 'freeform') {
        return reply(db, input.campaign_id, {
          ...
          report,
          message: `The restated "${entry.name}" is above the power budget, and a revision is not something the player can be asked to accept. Cut it down to what they already have, or offer the extra as a new boon with propose_feature.`,
        });
      }
  ...
  server.registerTool(
    'create_background',
    {
      ...
      inputSchema: {
        ...
        text: z.string().describe('What this background is, in a sentence or two.'),
      },
      annotations: { ...WRITES },
    },
    (input) => {
  ...
  server.registerTool(
    'propose_subclass',
    {
      ...
      inputSchema: {
        ...
        justification: z.string().describe('Why this fits the story and how they play; the player sees it in the dialog.'),
        recreated_from: z
          .string()
          .optional()
          .describe('The official subclass this one recreates, e.g. "Oath of Vengeance"; a label, not its text.'),
        allow_over_budget: z
          .boolean()
          .optional()
          .describe('The player asked for it knowing it is too strong. Never set this on your own.'),
  ...
  server.registerTool(
    'propose_spell',
    {
      title: 'Propose a custom spell',
      description:
        'Prices a spell you wrote against the SRD spells of the same level - a per-level budget taken from what those spells actually roll, single target and area counted apart, healing likewise, and a spell that only changes the fiction priced as a flat small cost - and then refuses it (strict), asks the player (flexible) or stores it with a warning (freeform). Write it as data, not prose: level, school, casting time, range, components, duration, and an effect saying whether it is an attack, a save, automatic, healing or utility, with the damage dice and type, the saving throw ability, the area shape, the healing dice or the condition it leaves. That data is what the engine executes, so use_action resolves the spell by name in a fight - the save DC comes from the caster sheet and the slot is spent the usual way. Pass character_id to also put it on that character sheet (a cantrip among their cantrips, anything else among their prepared spells); without it the spell is only stored for the campaign. Set allow_over_budget only when the player asked for something deliberately too strong.',
      inputSchema: {
        campaign_id: z.number().int(),
        character_id: CHARACTER_ID,
        schema: SPELL_SCHEMA,
