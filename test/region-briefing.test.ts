...
describe('regionBriefing on the safe realm', () => {
  ...
  it('shows the realm, its capital and counties, and the county of each settlement', () => {
    ...
    // Updated for the hierarchy: the realm is a header line and its counties follow it.
    expect(text).toContain('Kingdom of Ficengwind (kingdom, capital Ficengwind)');
    expect(text).toContain('  Outside duchies: County of Redham, County of Ficengwind');
...
describe('regionBriefing on the dangerous realm', () => {
  it('names the crownless realm and puts each danger in its county', () => {
    ...
    expect(text).toContain('Ta Isle (kingdom, no crown)');
    expect(text).toContain('  Outside duchies: County of Frostcot, County of Crimson Wharf');
